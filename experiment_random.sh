...
do
    python3 ResNet-train.py --classifier full --tensorization $tensorization --ranks "$line" --seed $seed --device $device | tee ./logs/resnet_trl_tensorized_full.$seed.$COUNTER.log
    COUNTER=$((COUNTER+1))
done